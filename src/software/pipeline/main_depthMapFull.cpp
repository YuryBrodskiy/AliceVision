...
int main(int argc, char* argv[])
{
    ...
    requiredParams.add_options()("ini", po::value<std::string>(&iniFilepath)->required(),
                                 "Configuration file (mvs.ini).")(
        "output,o", po::value<std::string>(&depthMapFolder)->required(), "Output folder for generated depth maps.");

    po::options_description optionalParams("Optional parameters");
    optionalParams.add_options()("rangeStart", po::value<int>(&rangeStart)->default_value(rangeStart),
        ...
        "Image downscale factor.")("sgmMaxTCams", po::value<int>(&sgmMaxTCams)->default_value(sgmMaxTCams),
        ...
        "Semi Global Matching: GammaC threshold.")("sgmGammaP", po::value<double>(&sgmGammaP)->default_value(sgmGammaP),
                                                   "Semi Global Matching: GammaP threshold.")(
        "refineNSamplesHalf", po::value<int>(&refineNSamplesHalf)->default_value(refineNSamplesHalf),
        "Refine: Number of samples.")("refineNDepthsToRefine",
                                      po::value<int>(&refineNDepthsToRefine)->default_value(refineNDepthsToRefine),
                                      "Refine: Number of depths.")(
        "refineNiters", po::value<int>(&refineNiters)->default_value(refineNiters),
        "Refine: Number of iterations.")("refineWSH", po::value<int>(&refineWSH)->default_value(refineWSH),
        ...
        "refineSigma", po::value<double>(&refineSigma)->default_value(refineSigma), "Refine: Sigma threshold.")(
        "refineGammaC", po::value<double>(&refineGammaC)->default_value(refineGammaC), "Refine: GammaC threshold.")(
        "refineGammaP", po::value<double>(&refineGammaP)->default_value(refineGammaP), "Refine: GammaP threshold.")(
	...
	po::options_description requiredParamsFilter("Required parameters");
    requiredParams.add_options() 
	("depthMapFilterFolder",					po::value<std::string>(&depthMapFilterFolder)->required(),															"Output folder for filtered depth maps.");
    po::options_description optionalParamsFilter("Optional parameters");
    optionalParams.add_options()
    ("minNumOfConsistensCams",					po::value<int>(&minNumOfConsistensCams)->default_value(minNumOfConsistensCams),										"Minimal number of consistent cameras to consider the pixel.")
    ("minNumOfConsistensCamsWithLowSimilarity", po::value<int>(&minNumOfConsistensCamsWithLowSimilarity)->default_value(minNumOfConsistensCamsWithLowSimilarity),	"Minimal number of consistent cameras to consider the pixel when the similarity is weak or ambiguous.")
    ("pixSizeBall",								po::value<int>(&pixSizeBall)->default_value(pixSizeBall),															"Filter ball size (in px).")
    ("pixSizeBallWithLowSimilarity",			po::value<int>(&pixSizeBallWithLowSimilarity)->default_value(pixSizeBallWithLowSimilarity),							"Filter ball size (in px) when the similarity is weak or ambiguous.")
    ("nNearestCams",							po::value<int>(&nNearestCams)->default_value(nNearestCams),															"Number of nearest cameras.");


    allParams.add(requiredParams).add(optionalParams).add(logParams).add(requiredParamsFilter).add(optionalParams);
    ...
    mvsUtils::MultiViewParams mp(iniFilepath, depthMapFolder, "", false, downscale);
